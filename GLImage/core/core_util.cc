...
std::string wx_GetFullPath(const char* filename)
{
    char runPath[MAX_PATH];
    GetModuleFileNameA(GetSelfModuleHandle()/*NULL*/, runPath, MAX_PATH);
    std::string fullpath = std::string(runPath);
    int found = fullpath.find_last_of("/\\");
    std::string ss =  fullpath.substr(0, found+1) + std::string(filename);
    return ss;
}
#endif
...
#else
    std::string path = wx_GetFullPath(name.c_str());
#endif
  return path;
}
#if defined(GPUPIXEL_IOS) || defined(GPUPIXEL_MAC)
